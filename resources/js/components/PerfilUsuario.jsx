            ...
            <div className="grid gap-6 lg:grid-cols-12">
                ...
                <Card className="lg:col-span-5" glow>
                    <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
                        ...
                        <div className="min-w-0 flex-1 text-center sm:text-left">
                            ...
                            <p className="mt-1 text-sm text-violet-300/80">Jugador activo · Perfil público</p>
                            <div className="mt-4">
                                <BarraXP
                ...
                <div className="grid gap-4 sm:grid-cols-3 lg:col-span-7">
                    <StatPill
                        icon={Target}
                        label="Aciertos"
                        value={`${usuario.aciertos_pct}%`}
                        sub="Precisión global"
                    />
                    <StatPill
                        icon={Timer}
                        label="Tiempo promedio"
                        value={`${usuario.tiempo_promedio_seg}s`}
                        sub="Por partida"
                    />
                    <StatPill
                        icon={Gamepad2}
                    ...
                    {statsRank ? (
                        <div className="space-y-4">
                            <div className="rounded-xl border border-blue-500/25 bg-blue-950/30 p-4">
                                <p className="text-3xl font-black text-blue-300">#{statsRank.pos}</p>
                                <p className="mt-1 text-sm text-slate-400">de {statsRank.total} jugadores</p>
                                <p className="mt-3 text-sm leading-relaxed text-slate-300">{mensajeTop}</p>
                            </div>
                            <div className="space-y-2 rounded-xl border border-slate-700/50 bg-slate-900/50 p-4 text-sm">
                                <p className="flex justify-between text-slate-400">
                                    <span>Tu puntaje</span>
                                    <span className="font-mono font-bold text-white">{statsRank.miPuntaje.toLocaleString()}</span>
                                </p>
                                {statsRank.siguiente && (
                                    <p className="flex justify-between border-t border-slate-700/50 pt-2 text-slate-400">
                                        <span>Siguiente ({statsRank.siguiente.nombre})</span>
                                        <span className="font-mono text-amber-200/90">
                                            +{statsRank.diffSiguiente} pts para superarle
                                        </span>
                                    </p>
                                )}
                                {!statsRank.siguiente && (
                                    <p className="border-t border-slate-700/50 pt-2 text-center text-emerald-400/90">
                                        ¡Líder del ranking!
                                    </p>
                                )}
                            </div>
